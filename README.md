...
```text
.env.example                 # пример переменных для загрузки модели и runtime
docs/
└── architecture-v1-v2.png  # общая схема архитектуры двух решений
scripts/
...
![Архитектура решений v1_ocr и v2_img3](docs/architecture-v1-v2.png)

Оба решения используют общий pipeline подготовки данных, базовую
`google/gemma-4-E4B-it`, общий LoRA-адаптер категории `БАД`, детерминированный
генератор comments v4 и один offline submission runtime. После дедупликации
данные разбиваются на train/validation, меньший класс oversample-ится только в
train, а для каждой категории сохраняются validation predictions, параметры
инференса и hashes артефактов.

Ветка категории `Легковоспламеняющиеся` различается:

- `v1_ocr` берёт первое фото, распознаёт его через EasyOCR и передаёт изображение
  вместе с OCR-текстом в Gemma; production использует агрегацию `single`;
- `v2_img3` независимо оценивает до трёх первых фотографий без OCR во входе
  классификатора и использует максимальную вероятность; EasyOCR участвует только
  в построении комментария.

После классификации verdict не изменяется правилами комментариев. Rule engine
объединяет название, описание и доступные OCR-свидетельства, формирует
валидированный комментарий длиной 50–300 символов и записывает итог в столбец
...
Репозиторий содержит [`.env.example`](.env.example) с переменными, которые
используются training downloader и offline runtime:

| Переменная | Обязательность | Назначение |
|---|---|---|
| `HF_TOKEN` | опционально | доступ к gated snapshot `google/gemma-4-E4B-it` при загрузке |
| `SHARED_MODELS_PATH` | опционально | корень shared-моделей; runtime по умолчанию использует `/shared_models` |
| `ECUP_BATCH_SIZE` | опционально | переопределение batch size инференса; по умолчанию `2` |

Python-код намеренно не загружает `.env` автоматически. Для локального запуска
скопируйте пример и экспортируйте значения в текущую оболочку:

```bash
cp .env.example .env
# заполните нужные значения в .env
set -a
source .env
set +a
```

Для Docker тот же файл можно передать через `--env-file .env`. Не добавляйте
рабочий `.env` с токенами в Git; он уже исключён через `.gitignore`.
